...
export function buildCustomProperties(themeConfig, globalTheming) {
    return globalTheming
        ? Object.assign({}, buildLegacyColors(themeConfig), buildColors(themeConfig)) : Object.assign({}, buildLegacyColors(themeConfig));
}
export function buildThemeContext(themeConfig) {
...
export function buildColors(theme) {
    ...
    const { surface = '#FAFAFA', onSurface = '#1F2225', interactive = '#0870D9', interactiveNeutral = '#EAEAEB', branded = '#008060', critical = '#E32727', warning = '#FFC453', highlight = '#59D0C2', success = '#008060', } = UNSTABLE_colors;
...
function customPropertyTransformer(colors) {
    return Object.entries(colors).reduce((transformed, [key, value]) => (Object.assign({}, transformed, { [toCssCustomPropertySyntax(key)]: hslToString(value) })), {});
}
function toCssCustomPropertySyntax(camelCase) {
    return `--p-${camelCase.replace(/([A-Z0-9])/g, '-$1').toLowerCase()}`;
}
function setLightness({ hue, saturation, alpha }, lightness) {
